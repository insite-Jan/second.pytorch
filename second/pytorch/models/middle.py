...
def register_middle(cls, name=None):
    ...
    if name is None:
        name = cls.__name__
    assert name not in REGISTERED_MIDDLE_CLASSES, "exist class: {}".format(REGISTERED_MIDDLE_CLASSES)
    REGISTERED_MIDDLE_CLASSES[name] = cls
    return cls

def get_middle_class(name):
    global REGISTERED_MIDDLE_CLASSES
    assert name in REGISTERED_MIDDLE_CLASSES, "available class: {}".format(REGISTERED_MIDDLE_CLASSES)
